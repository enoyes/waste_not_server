  ...
  def sync_data
    ...
    new_food_items.each do |k, v|
      ...
      if fi
        ...
        fi = FoodItem.new(f_id: v[:f_id], name: v[:name], desc: v[:desc], purchased: v[:purchased], expiration: v[:expiration], size_value: v[:size_value], size_unit: v[:size_unit], remaining: v[:remaining], public: v[:public])
